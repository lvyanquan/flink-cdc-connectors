 ...
 *
 * @param <Event> the type of the committable
 * @param <CommT> the type of the committable (to send to downstream operators)
 */
class DataSinkWriterOperator<CommT> extends AbstractStreamOperator<CommittableMessage<CommT>>
    ...
    private final Context<Event> context;
    ...
    private SinkWriter<Event> sinkWriter;

    private final SinkWriterStateHandler<Event> writerStateHandler;
    ...
    DataSinkWriterOperator(Sink<Event> sink) {
        ...
        if (sink instanceof StatefulSink) {
            writerStateHandler =
                    new StatefulSinkWriterStateHandler<>((StatefulSink<Event, ?>) sink);
        } else {
        ...
        if (sink instanceof TwoPhaseCommittingSink) {
            committableSerializer =
                    ((TwoPhaseCommittingSink<Event, CommT>) sink).getCommittableSerializer();
        } else {
